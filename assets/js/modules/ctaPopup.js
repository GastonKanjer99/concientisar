export function setupCTAPopup({
  ...
  const openToast = () => {
    ...
    const close = () => { el.classList.remove("show"); setTimeout(()=>el.remove(),180); store.setItem(storageKey,"1"); };
  ...
  const openModal = () => {
    ...
    const firstBtn = modal.querySelector(".btn"); firstBtn?.focus();

    const close = () => {
      backdrop.classList.remove("show"); modal.classList.remove("show");
      setTimeout(()=>{ backdrop.remove(); modal.remove(); unlockScroll(); prevFocus?.focus(); }, 200);
